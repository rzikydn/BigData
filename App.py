...
# ===== Tab 1: Overview =====
with tab1:
    df_tab1 = df_bigdata.copy()

    st.subheader("VISUALISASI DATA SERTIFIKASI BY BASYS")

    #-- Filter Tanggal--#
    min_date = df_tab1["date certification"].min().date()
    max_date = df_tab1["date certification"].max().date()
    ...
    jenis_list = ["All"] + sorted(df_tab1["jenis sertifikasi"].dropna().unique())
    instansi_list = ["All"] + sorted(df_tab1["instansi"].dropna().unique())
    ...
    # Filter data
    filtered_df = df_tab1[
        (df_tab1["date certification"].dt.date >= start_date) &
        (df_tab1["date certification"].dt.date <= end_date)
    ]
    if sel_jenis != "All":
    ...
    st.plotly_chart(fig_over, use_container_width=True, key="bar_tab1")
...
# ===== Tab 2: By Notion =====
with tab2:
    df_tab2_notion = df_notion.copy()
    df_tab2_bigdata = df_bigdata.copy()

    st.subheader("VISUALISASI DATA PERBANDINGAN DATA NOTION & BASYS")

    # === Dropdown filter berdasarkan nama sertifikasi (Notion) ===
    sertifikasi_options = ["Semua"] + sorted(df_tab2_notion["nama sertifikasi"].dropna().unique().tolist())
    ...
    if selected_sertifikasi != "Semua":
        df_notion_filtered = df_tab2_notion[df_tab2_notion["nama sertifikasi"] == selected_sertifikasi]
    else:
        df_notion_filtered = df_tab2_notion.copy()
    ...
    filtered_bigdata_same_date = df_tab2_bigdata[
        (df_tab2_bigdata["date certification"].dt.date >= start_date) &
        (df_tab2_bigdata["date certification"].dt.date <= end_date)
    ]
    if selected_sertifikasi != "Semua":
    ...
    df_tab2_bigdata["selesai"] = pd.to_numeric(df_tab2_bigdata["selesai"], errors="coerce").fillna(0)
    ...
    st.plotly_chart(fig_line, use_container_width=True, key="line_tab2")

    with st.expander("🔽 FUNGSI BAGIAN INI", expanded=True):
        st.markdown("""
        Bagian ini menampilkan perbandingan data Notion dan Basys.
        """)

# ===== Tab 3: By Institution =====
with tab3:
    df_tab3 = df_bigdata.copy()

    st.subheader("VISUALISASI DATA TOP 5 INSTANSI")
 
    min_date_inst = df_tab3["date certification"].min().date()
    max_date_inst = df_tab3["date certification"].max().date()
    ...
    # Filter data
    filtered_df_inst = df_tab3[
        (df_tab3["date certification"].dt.date >= start_date) &
        (df_tab3["date certification"].dt.date <= end_date)
    ]

    jenis_list_inst = ["All"] + sorted(df_tab3["jenis sertifikasi"].dropna().unique())
    ...
    st.plotly_chart(fig_lolli, use_container_width=True, key="lolli_tab3")

    with st.expander("🔽 Fungsi Bagian Ini", expanded=True):
        st.markdown("""
        Menampilkan 5 instansi dengan jumlah pendaftar terbanyak.
        """)
